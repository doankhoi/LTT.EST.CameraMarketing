...
Enviroment::Enviroment()
{
	...
	//Bốn điểm danh giới
	BOTTOM_LEFT = cv::Point( static_cast<int>(FRAME_WIDTH* SCALE_WIDTH) - 30, static_cast<int> (FRAME_HEIGHT * SCALE_HEIGHT) + 10);

	BOTTOM_RIGHT = cv::Point(FRAME_WIDTH - static_cast<int>(static_cast<int>(FRAME_WIDTH* SCALE_WIDTH) + 10), static_cast<int> (FRAME_HEIGHT * SCALE_HEIGHT) + 10);

	TOP_LEFT = cv::Point(static_cast<int>(FRAME_WIDTH* SCALE_WIDTH) - 30,static_cast<int>(FRAME_HEIGHT * 0.05));
	TOP_RIGHT = cv::Point(FRAME_WIDTH - static_cast<int>(static_cast<int>(FRAME_WIDTH* SCALE_WIDTH) + 10), static_cast<int>(FRAME_HEIGHT * 0.05));
	...
	//Đặt ngưỡng y có thể chấp nhận
	THRESHOLD_Y_ACCEPT = BOTTOM_LEFT.y - 55;
}
...
void Enviroment::drawEnviroment(cv::Mat& frame)
{
	...
	//cv::line(frame, Point(TOP_LEFT.x, THRESHOLD_Y_ACCEPT), Point(TOP_RIGHT.x, THRESHOLD_Y_ACCEPT), cv::Scalar(255, 0, 0), 2);
}
